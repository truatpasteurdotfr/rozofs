...
from rozofs.core.constants import LAYOUT, STORAGES, STORAGE_SID, STORAGE_ROOT, \
...
class StorageConfig():
    def __init__(self, sid=0, root=""):
        self.sid = sid
        self.root = root


class StoragedConfig():
    def __init__(self, layout=LAYOUT_2_3_4, ports=[], storages={}):
        self.layout = layout
...
class StoragedConfigurationParser(ConfigurationParser):
    ...
    def unparse(self, config, configuration):
        ...
        for i in range(config_setting_length(storage_settings)):
            ...
            configuration.storages[sid] = StorageConfig(sid, root)
