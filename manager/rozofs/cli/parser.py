...
from rozofs.core.constants import PLATFORM_MANAGER, EXPORTD_MANAGER, \
    STORAGED_MANAGER, SHARE_MANAGER, PROTOCOLS_VALUES
...
__parser.add_argument('listeners', nargs='+', choices=[PLATFORM_MANAGER, EXPORTD_MANAGER, STORAGED_MANAGER, SHARE_MANAGER], help='list of listeners.')
__parser = __add_command_parser('agent-stop', 'stop a rozo agent.', agent_dispatch)


__parent = argparse.ArgumentParser(add_help=False)
__parent.add_argument('-H', '--host', default='localhost', help='running platform agent host')
...
# profiler management
__parser = __add_command_parser('profile', 'display nodes profiler(s).', platform_dispatch, [__parent])
...
# config management
__parser = __add_command_parser('configuration', 'display nodes configurations.', platform_dispatch, [__parent])
__parser.add_argument('-n', '--nodes', nargs='+', help='list of nodes to be displayed. if not set all nodes will be displayed')
__parser.add_argument('-r', '--roles', nargs='+', choices=STR_ROLES.keys(), help='list of roles to be displayed for each nodes. if not set all roles will be displayed')

__parser = __add_command_parser('set-layout', 'set platform layout.', platform_dispatch, [__parent])
...
__parser = __add_command_parser('add-nodes', 'add nodes to the platform.', platform_dispatch, [__parent])
...
__parser = __add_command_parser('remove-volume', 'remove volume(s) from the platform.', platform_dispatch, [__parent])
__parser.add_argument('vids', nargs="+", type=int, help='vid(s) of existing volume.')

__parser = __add_command_parser('add-export', 'export a new file system.', platform_dispatch, [__parent])
...
__parser = __add_command_parser('update-export', 'modify an exported file system.', platform_dispatch, [__parent])
__parser.add_argument('-p', '--passwd', default=None, help='password to set.')
...
__parser = __add_command_parser('remove-export', 'remove exported file system(s).', platform_dispatch, [__parent])
__parser.add_argument('-f', '--force', action="store_true", default=False, help='when ever to force removing.')
__parser.add_argument('eids', nargs="+", type=int, help='eid(s) of existing export.')
