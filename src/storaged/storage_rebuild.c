    ...
    for (spare_it = 0; spare_it < 2; spare_it++) {
	  ...
	  // Check whether this file should have a header(mapper) file on 
 	    ...
 	    if (dev == device_to_rebuild) {
          ...
          // Check whether this file has some chink of data on the device to rebuild
...
int rbs_rebuild_storage(const char *export_host_list, int site, cid_t cid, sid_t sid,
    ...
    if (rb_fid_table_count==0) {
      ...
      while (ret != 0) {
        ...
        run_loop++;
	ret = rbs_do_list_rebuild(cid,sid);
...
static inline void * rebuild_storage_thread(rbs_stor_config_t *stor_confs) {
    ...
    */
    ...
    for (rbs_index = 0; rbs_index < nb_rbs_entry; rbs_index++) {
        ...
        if (stor_confs[rbs_index].cid == 0 && stor_confs[rbs_index].sid == 0) {
            continue;
        }
	...
	result = -1;
        while (result == -1) {
            ...
            REBUILD_MSG("Start rebuild process for storage (cid=%u;sid=%u).",
	                 stor_confs[rbs_index].cid, stor_confs[rbs_index].sid);
	    	    	    	
	    result = rbs_rebuild_storage(stor_confs[rbs_index].export_hostname, 
	        		       storaged_geosite,
                		       stor_confs[rbs_index].cid, stor_confs[rbs_index].sid, stor_confs[rbs_index].root,
...
int main(int argc, char *argv[]) {
    ...
    */
    if (rbs_start_process == 0){
        REBUILD_FAILED("Missing --rebuild option.");
        exit(EXIT_FAILURE);
    } 
    /*
