   ...
   */
   ...
   while(1)
   {
...

    {
        ...
        {
          sprintf(name, "stor_io %s:%d ", arg_p->hostname,arg_p->io_port);        
        }
...
int storaged_start_nb_blocking_th(void *args) {
    ...
    */
    ...
    info("Storaged instance %d non blocking thread started:   %s:%d",args_p->instance_id,args_p->hostname,args_p->debug_port);
