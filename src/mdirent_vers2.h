...
static inline void *dirent_malloc(int size,int line)
{
   ...
   p = malloc(size+8);
   if (p == NULL) printf("Out of memory at line %d\n",line);
...
static inline void dirent_mem_print_stats_per_size()
{
  ...
  {
     if (malloc_size_tb[i] == 0) continue;
...
{
  memset(dirent_coll_stats_tb,0,sizeof(uint64_t)*DIRENT_COLL_STAT_MAX_IDX);
...
static inline void dirent_cache_lookup_print_stats()
{
  ...
  {
     if (dirent_coll_stats_tb[i] == 0) continue;
     printf("Count with %8.8d  Collisions: %llu\n",1<<i,(long long unsigned int)dirent_coll_stats_tb[i]);
  }
   printf("Total Lookups :%llu  Total Collisions :%llu average collisions counter :%llu\n",
           (long long unsigned int)dirent_coll_stats_cumul_lookups,
           (long long unsigned int)dirent_coll_stats_cumul_collisions,
           (dirent_coll_stats_cumul_lookups==0)?0:(long long unsigned int)dirent_coll_stats_cumul_collisions/dirent_coll_stats_cumul_lookups);
...
*/
static inline ssize_t dirent_pread(int fd, void *buf, size_t count, off_t offset)
{
...
*/
static inline ssize_t dirent_pwrite(int fd, const void *buf, size_t count, off_t offset)
{
...
{
  uint8_t   u8[((sizeof(mdirents_header_new_t)+sizeof(mdirents_btmap_free_hash_t)-1)/MDIRENT_SECTOR_SIZE+1)*MDIRENT_SECTOR_SIZE];
...
{
  uint8_t   u8[((sizeof(mdirents_btmap_free_chunk_t)-1)/MDIRENT_SECTOR_SIZE+1)*MDIRENT_SECTOR_SIZE];
  struct {
    mdirents_btmap_free_chunk_t name_bitmap;  ///< bitmap of the free name/fid/type entries
  }s;
} mdirent_sector1_t;
...
{
  uint8_t   u8[((sizeof(mdirents_hash_tab_t)-1)/MDIRENT_SECTOR_SIZE+1)*MDIRENT_SECTOR_SIZE];
  struct {
    mdirents_hash_tab_t        hash_tbl;     ///< hash table: set of of 256 logical pointers   
  }s;
} mdirent_sector2_t;
...
{
  uint8_t   u8[((sizeof(mdirents_hash_entry_t)*MDIRENTS_ENTRIES_COUNT-1)/MDIRENT_SECTOR_SIZE+1)*MDIRENT_SECTOR_SIZE];
  struct {
    mdirents_hash_entry_t      hash_entry[MDIRENTS_ENTRIES_COUNT];   ///< table of hash entries   
  }s;
} mdirent_sector3_t;
...
static inline char *dirent_build_filename(mdirents_header_new_t *p,char *buf)
{
   char *pdata;
   int i;
   if (p->level_index > MDIRENTS_MAX_COLL_LEVEL) return NULL;
   pdata = buf;
   pdata+=sprintf(pdata,"d");
   for (i = 0; i < p->level_index+1 ; i++) pdata+=sprintf(pdata,"_%d",p->dirent_idx[i]);
   return buf;   
} 
/**
...
*/     
static inline int check_bytes(uint8_t *p,int first_chunk,int last_chunk,int *loop_cnt)
{
...
*  That function go throught the bitmap of free chunk by skipping allocated chunks
   ...
   @retval next_chunk !=0 next free chunk bit to test
*/     
static inline int check_bytes_val(uint8_t *p,int first_chunk,int last_chunk,int *loop_cnt,uint8_t empty)
{
  int chunk_idx=first_chunk;
  int chunk_u8_idx=0;
  int align;
...
*/
static inline int dirent_allocate_chunks(uint8_t nb_chunks,uint8_t *p,int last_chunk)
{
    ...
    while (chunk_idx < last_chunk)
    {
      ...
      if ((p[chunk_u8_idx] & (1<<bit_idx)) == 0)
      {
        ...
        */
        if (chunk_idx%8 == 0)
        {
          next_chunk_idx = check_bytes_val(p,chunk_idx,last_chunk,&loop_cnt,0);
          if (next_chunk_idx < 0) break;
          /*
          ** next  chunk
          */
          if (next_chunk_idx == chunk_idx) chunk_idx++;  
          else  chunk_idx = next_chunk_idx;
          continue; 
        }
        /*
        ** next chunk
...
*  API to Get the next chunk whose bit value is indicated by val
  ...
  @retval =-1 -> allocation failure
*/
static inline int dirent_getnext_chunk(int  first_chunk,uint8_t *p,int last_chunk,int val)
{
    uint8_t  chunk_u8_idx ;
    int  chunk_idx = first_chunk ;
    int  next_chunk_idx = 0 ;
    ...
    while (chunk_idx < last_chunk)
    {
      ...
      if ( bit_res != val)
      {
        ...
        */
        if (chunk_idx%8 == 0)
        {
          next_chunk_idx = check_bytes_val(p,chunk_idx,last_chunk,&loop_cnt,(1-val));
          if (next_chunk_idx < 0) break;
          /*
          ** next  chunk
          */
...
*  API to allocate a consecutive number of chunks with a given alignment
  ...
  @retval =-1 -> allocation failure
*/
static inline int dirent_allocate_chunks_with_alignment(uint8_t nb_chunks,uint32_t alignment,uint8_t *p,int last_chunk)
{
    uint8_t  chunk_u8_idx ;
    int  chunk_idx = 0 ;
    int  next_chunk_idx = 0 ;
...
static inline int dirent_release_chunks(int start_chunk,uint8_t nb_chunks,uint8_t *p,int last_chunk)
{
   int k;
   int chunk_u8_idx;
   int bit_idx;

   if ((start_chunk+nb_chunks) > last_chunk) return -1;
   /*
   ** OK, now proceed with the reservation of the bits
   */
...
static inline int dirent_check_empty_chunks(int start_chunk,uint8_t nb_chunks,uint8_t *p,int last_chunk)
{
   int i;
   int start_u8_chunk;
   int loop_cnt;
   ...
   */
   ...
   {
     if (p[start_u8_chunk+i] != 0xff) return 0;
   }
   return 1;
}
...
static inline void *dirent_cache_get_entry_base_ptr(mdirent_cache_ptr_t *p,
                                              uint16_t nb_elem_per_array,
                                              uint16_t max_idx,
                                              uint16_t idx,uint16_t *idx_rel_p,
                                              mdirent_cache_ptr_t **virt_p)
{
   uint16_t idx_array;
   
   if(idx >= (max_idx*nb_elem_per_array)) return NULL;
                                         ...
                                         mdirent_cache_ptr_t **virt_p)
{
   ...
   elem_p = (uint8_t*)dirent_cache_get_entry_base_ptr(p,nb_elem_per_array,max_idx,idx,&rel_idx,virt_p);
   if (elem_p == (uint8_t*)NULL) return (void*) NULL;
                                         ...
                                         uint16_t idx)
{
   ...
   elem_p = (uint8_t*)dirent_cache_get_entry_base_ptr(p,nb_elem_per_array,max_idx,idx,&rel_idx,NULL);
...
the hash bucket entries
  ...
  @retval -1 on error
*/
static inline void dirent_cache_release_all_hash_tbl_memory(mdirents_cache_entry_t *dirent_entry_p)
{
...
static inline int dirent_get_cache_indexes(uint32_t idx,uint8_t *idx_tab_res_p,mdirent_indirect_ptr_template_t *p)
{
   int          output_idx = 0;
   uint32_t     shift;
   uint32_t     val;
   ...
   if (idx > (p->last_idx-1)) return  -1;
   shift = 0;
   for (i = 0; i < p->levels; i++) shift += p->level[i];
...
static inline void *dirent_cache_store_ptr(mdirent_cache_ptr_t *virt_head_p,
                                         mdirent_indirect_ptr_template_t *indirection_template_p,
                                         int idx,void *ptr)
{
   int res;
      ...
      */
      if (p[tab_idx[i]].s.val == 0) 
      {
         ...
         */
         ...
         int size = (1<<indirection_template_p->level[i+1])*sizeof(mdirent_cache_ptr_t);
...
*
   ...
   @retval <> NULL pointer to object associated with idx
*/
static inline mdirent_cache_ptr_t dirent_cache_get_virtual_ptr(mdirent_cache_ptr_t *virt_head_p,
                                                               mdirent_indirect_ptr_template_t *indirection_template_p,
                                                               int idx)
{
   int res;
...
*
  ...
  retval 0 on success
*/
static inline void dirent_cache_release_name_all_memory(mdirents_cache_entry_t *dirent_entry_p)
{
...
static inline void *dirent_get_entry_name_ptr( int dir_fd, mdirents_cache_entry_t *dirent_entry_p, uint32_t start_chunk_idx, int alloc_req)
{
   int            ret;
   uint32_t first_chunk_of_array = start_chunk_idx/MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY;
   uint32_t chunk_offset         = start_chunk_idx - first_chunk_of_array*MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY;
   uint8_t *mem_p;

   /*
   ** attempt to get a valid pointer from memory
   */
   mem_p = (uint8_t *) dirent_cache_get_ptr(&dirent_entry_p->name_entry_lvl0_p[0],
                                            &mdirent_cache_name_ptr_distrib,
   ...
   */
   ...
   ret = dirent_test_chunk_bit(first_chunk_of_array,dirent_entry_p->name_entry_array_btmap_presence);
   if (ret != 0)
   {
     /*
     ...
     */
     ...
     ret = dirent_read_name_array_from_disk(dir_fd,dirent_entry_p,first_chunk_of_array);
     if (ret == -1)
     {
        /*
...
static inline void *dirent_create_entry_name( int dir_fd, mdirents_cache_entry_t *dirent_entry_p,
                                               ...
                                               mdirents_hash_entry_t *hash_entry_p)
{
   ...
   */
   ...
   dest_mem_p = (uint32_t*) dirent_get_entry_name_ptr(dir_fd,dirent_entry_p,start_chunk,DIRENT_CHUNK_ALLOC);
   if (dest_mem_p == NULL)
   {
     /*
   ...
   */
   for (i = 0; i < (((entry_size-1)>>2)+1); i++) dest_mem_p[i] =  *src_mem_p++;
   /*
   ** indicates that the memory array must be updated on disk 
   */
   first_chunk_of_array = start_chunk/MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY;
   dirent_set_chunk_bit(first_chunk_of_array,dirent_entry_p->name_entry_array_btmap_wr_req);
...
*
   ...
   @retval -1 on error :
*/
static inline int dirent_cache_del_entry_name(mdirents_cache_entry_t *dirent_entry_p, 
                                              uint32_t start_chunk_idx,
                                              uint32_t nb_chunks)
{
   ...
   */
   ret = DIRENT_CACHE_RELEASE_NAME_ENTRY_CHUNKS(mem_p,start_chunk_idx,nb_chunks);
   if (mem_p == NULL)
   {
      /*
      ** something wrong in the cache entry context
   ...
   */
   {
     ...
     first_chunk_of_array = start_chunk_idx/MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY;
     int i;

     for (i = 0; i < MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY/8; i++)
     {
        if (mem_p[(first_chunk_of_array*(MDIRENTS_CACHE_NB_CHUNK_PER_CHUNK_ARRAY/8))+i] != 0xff)
        {
          /*
   ...
   */
   dirent_clear_chunk_bit(first_chunk_of_array,dirent_entry_p->name_entry_array_btmap_presence);
   dirent_clear_chunk_bit(first_chunk_of_array,dirent_entry_p->name_entry_array_btmap_wr_req);
   ...
   */
   virt_ptr = dirent_cache_get_virtual_ptr(&dirent_entry_p->name_entry_lvl0_p[0],
                                                                   &mdirent_cache_name_ptr_distrib,
                                                                   first_chunk_of_array);
   if (virt_ptr.s.val == 0)
   {
...
*/
static inline int dirent_cache_entry_check_empty(mdirents_cache_entry_t *dirent_entry_p)
{
    ...
    */
    ...
    {
      if (hash_bitmap_p[i] != val) return 0;
    }
    /*
    ...
    */
    if (sect0_p->header.level_index == 0)
    {
      ...
      */
      ...
      {
        if (coll_bitmap_p[i] != val) return 0;      
      }    
    }
    /*
...
*   The purpose of that API is to check if the collision bitmap of the root is empty (DEBUG Function)
  ...
  @retval -1 : sector0 pointer does not exist
*/
static inline int dirent_cache_check_coll_bitmap_empty(mdirents_cache_entry_t *root_p)
{
    mdirent_sector0_not_aligned_t *sect0_p;
    uint64_t            *coll_bitmap_p;
    int i;
...
the hash entries
  ...
  @retval -1 on error
*/
static inline void dirent_cache_release_all_hash_entries_memory(mdirents_cache_entry_t *dirent_entry_p)
{
    mdirent_cache_ptr_t *hash_entry_virt_mem_p = &dirent_entry_p->hash_entry_p[0];
    uint8_t  *mem_p;
    int      i;
...
*/
mdirents_cache_entry_t *dirent_cache_create_entry(mdirents_header_new_t *dirent_hdr_p);
...
*/
mdirents_cache_entry_t *dirent_cache_alloc_name_entry_idx( mdirents_cache_entry_t *parent,
                                                            int                   bucket_idx,
...
mdirents_cache_entry_t *dirent_cache_search_and_alloc_hash_entry ( mdirents_cache_entry_t *root,
                                                                   int                    bucket_idx,
                                                                   uint32_t               hash_value,
                                                                   int                   *hash_entry_match_idx_p,
                                                                   dirent_cache_search_alloc_t *cache_alloc_p);
...
mdirents_cache_entry_t *dirent_cache_delete_hash_entry ( int dir_fd,
                                                         ...
                                                         uint32_t               hash_value,
                                                         int                   *hash_entry_match_idx_p,
                                                         mdirents_cache_entry_t **cache_entry_prev_ret,
                                                         uint8_t               *name,
                                                         uint16_t              len,
                                                         void                  *fid,
                                                         uint32_t               *mode_p);
...
*
  ...
  @retval NULL: not found
*/
mdirents_cache_entry_t *dirent_cache_print_bucket_list( mdirents_cache_entry_t *root,
                                                         int                    bucket_idx);
/*
**______________________________________________________________________________
*/
...
static inline uint32_t  dirent_read_from_disk(mdirents_cache_entry_t *parent,uint32_t coll_idx)
{
  return 0;
...
static inline mdirents_cache_entry_t *dirent_cache_store_collision_ptr(mdirents_cache_entry_t *parent, mdirents_cache_entry_t *dirent_entry)
{
   int dirent_coll_file_idx;
   mdirents_cache_entry_t *returned_ptr;

   dirent_coll_file_idx = dirent_entry->header.dirent_idx[dirent_entry->header.level_index];
   returned_ptr = dirent_cache_store_ptr(&parent->dirent_coll_lvl0_p[0],
                                         &mdirent_cache_indirect_coll_distrib,
                                         dirent_coll_file_idx,(void *)dirent_entry);
   return returned_ptr;
...
*
   ...
   @retval <>NULL on error : value of the input pointer if the not reference in the indirection array, or value of
            the current entry if the entry does not match with the input pointer (ptr)
*/
static inline mdirents_cache_entry_t *dirent_cache_del_collision_ptr(mdirents_cache_entry_t *parent, 
                                                                     mdirents_cache_entry_t *dirent_entry)
{
   ...
   dirent_coll_file_idx = dirent_entry->header.dirent_idx[dirent_entry->header.level_index];
   returned_ptr = (mdirents_cache_entry_t*)dirent_cache_del_ptr(&parent->dirent_coll_lvl0_p[0],
                                                                &mdirent_cache_indirect_coll_distrib,
                                                                dirent_coll_file_idx,(void *)dirent_entry);
   /*
   ** set the entry free in the collision bitmap
   */
   dirent_set_chunk_bit(dirent_coll_file_idx,(uint8_t*)&sect0_p->coll_bitmap);
   /*
   ** need to update parent on disk
   */
...
static inline mdirents_cache_entry_t *dirent_cache_get_collision_ptr(mdirents_cache_entry_t *parent,uint32_t coll_idx)
{
 mdirent_cache_ptr_t virt_ptr;
 uint32_t            ret;
 /*
 ...
 */
 if ((virt_ptr.s.rd == 1) && (virt_ptr.s.dirty == 1))
 {
   ...
   ** the file has been loaded in memory and the virtual pointer has been updated in the parent cache entry, just need to get it
   */
   return (mdirents_cache_entry_t*)dirent_cache_get_ptr(&parent->dirent_coll_lvl0_p[0],
                                                         &mdirent_cache_indirect_coll_distrib,
                                                         coll_idx);   
 }
 /*
 ...
 */
mdirents_cache_entry_t * read_mdirents_file(int dirfd,mdirents_header_new_t *dirent_hdr_p);     
 ...
 * @retval -1 on error
 */
int  dirent_write_name_array_to_disk(int dirfd,mdirents_cache_entry_t *dirent_cache_p,uint16_t start_chunk_idx);
...
* Allocation of a dirent file cache entry
  ...
  @retval NULL : out of cache entries
*/
mdirents_cache_entry_t *dirent_cache_allocate_entry(mdirents_header_new_t *dirent_hdr_p);
...
typedef enum _dirent_file_repair_cause_e
{
 ...
 DIRENT_REPAIR_NO_COLL_FILE,
 DIRENT_REPAIR_NO_EOF,
 DIRENT_REPAIR_MAX
 ...
 * @retval -1 on failure
 */
void dirent_cache_repair_cache_entry_for_bucket_idx(int fd_dir,mdirents_cache_entry_t *root,
                                                    mdirents_cache_entry_t *cache_entry_p,int bucket_idx,
                                                    mdirents_hash_entry_t **hash_entry_last_p,int *first_index);
...
static inline dirent_file_repair_cause_e dirent_cache_check_repair_needed(mdirents_cache_entry_t *root,
   ...
   switch (hash_bucket_p->type)
   {
      case MDIRENTS_HASH_PTR_LOCAL:
        ...
        */
        ...
        if (dirent_test_chunk_bit(cur_hash_entry_idx_repair,hash_bitmap_p) != 0) 
        {
        ...
        */
        hash_entry_next_p = (mdirents_hash_entry_t*)DIRENT_CACHE_GET_HASH_ENTRY_PTR(cache_entry_p,cur_hash_entry_idx_repair);
        ...
        hash_entry_bucket_idx_next = DIRENT_HASH_ENTRY_GET_BUCKET_IDX(hash_entry_next_p);
      ...
      case MDIRENTS_HASH_PTR_COLL:
        ...
        ** collision file
        */
        cache_coll_entry = dirent_cache_get_collision_ptr(root,hash_bucket_p->idx);
 ...
 * @retval none
 */
void dirent_file_repair(int dir_fd ,mdirents_cache_entry_t *root_entry_p,int bucket_idx,dirent_file_repair_cause_e cause);
 ...
 * @retval none
 */
void dirent_file_check(int dir_fd ,mdirents_cache_entry_t *root_entry_p,int bucket_idx);
