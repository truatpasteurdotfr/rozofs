...
static int load_volumes_conf(econfig_t *ec, struct config_t *config) {
    ...
    if ((volumes_set = config_lookup(config, EVOLUMES)) == NULL) {
        errno = ENOKEY;
        fatal("can't lookup volumes setting.");
        goto out;
    }
    ...
    for (v = 0; v < config_setting_length(volumes_set); v++) {
        ...
        if ((vol_set = config_setting_get_elem(volumes_set, v)) == NULL) {
            errno = ENOKEY;
            fatal("can't get volume setting %d.", v);
            goto out;
        }
        ...
        if (config_setting_lookup_int(vol_set, EVID, &vid) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't lookup vid setting for volume idx: %d.", v);
            goto out;
        }
        ...
        if ((clu_list_set = config_setting_get_member(vol_set, ECIDS)) == NULL) {
            errno = ENOKEY;
            fatal("can't get cids setting for volume idx: %d.", v);
            goto out;
        }
        ...
        for (c = 0; c < config_setting_length(clu_list_set); c++) {
            ...
            if ((clu_set = config_setting_get_elem(clu_list_set, c)) == NULL) {
                errno = ENOKEY;
                fatal("can't get cluster setting for volume idx: %d cluster idx: %d.", v, c);
                goto out;
            }
            ...
            if (config_setting_lookup_int(clu_set, ECID, &cid) == CONFIG_FALSE) {
                errno = ENOKEY;
                fatal("can't lookup cid for volume idx: %d cluster idx: %d.", v, c);
                goto out;
            }
            ...
            for (s = 0; s < config_setting_length(stor_set); s++) {
                ...
                if ((mstor_set = config_setting_get_elem(stor_set, s)) == NULL) {
                    errno = ENOKEY;
                    fatal("can't get storage config for volume idx: %d\
                ...
                if (config_setting_lookup_int(mstor_set, ESID, &sid) == CONFIG_FALSE) {
                    errno = ENOKEY;
                    fatal("can't get sid for volume idx: %d\
                             , cluster idx: %d,  storage idx: %d.", v, c, s);
                    goto out;
                }
                ...
                if (config_setting_lookup_string(mstor_set, EHOST, &host) == CONFIG_FALSE) {
                    errno = ENOKEY;
                    fatal("can't get host for volume idx: %d\
                             , cluster idx: %d,  storage idx: %d.", v, c, s);
                    goto out;
                }
                ...
                if (storage_node_config_initialize(snconfig, (sid_t)sid, host) != 0) {
                    fatal("can't initialize storage node config.");
                }
...
static int load_exports_conf(econfig_t *ec, struct config_t *config) {
    ...
    if ((export_set = config_lookup(config, EEXPORTS)) == NULL) {
        errno = ENOKEY;
        fatal("can't lookup exports setting.");
        goto out;
    }
    ...
    for (i = 0; i < config_setting_length(export_set); i++) {
        ...
        if ((mfs_setting = config_setting_get_elem(export_set, i)) == NULL) {
            errno = ENOKEY;
            fatal("can't get export idx: %d.", i);
            goto out;
        }

        if (config_setting_lookup_int(mfs_setting, EEID, (long int *) &eid) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up eid for export idx: %d", i);
            goto out;
        }

        if (config_setting_lookup_string(mfs_setting, EROOT, &root) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up root path for export idx: %d", i);
            goto out;
        }

        if (config_setting_lookup_string(mfs_setting, EMD5, &md5) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up md5 for export idx: %d", i);
            goto out;
        }

        if (config_setting_lookup_string(mfs_setting, ESQUOTA, &str) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up squota for export idx: %d", i);
            goto out;
        }

        if (strquota_to_nbblocks(str, &squota) != 0) {
            fatal("%s: can't convert to quota)", str);
            goto out;
        }

        if (config_setting_lookup_string(mfs_setting, EHQUOTA, &str) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up hquota for export idx: %d", i);
            goto out;
        }
        ...
        if (config_setting_lookup_int(mfs_setting, EVID, &vid) == CONFIG_FALSE) {
            errno = ENOKEY;
            fatal("can't look up vid for export idx: %d", i);
            goto out;
        }

        econfig = xmalloc(sizeof(export_config_t));
        if (export_config_initialize(econfig, eid, vid, root, md5, squota,
                hquota) != 0) {
            fatal("can't initialize export config.");
        }
        // Initialize export
...
int econfig_read(econfig_t *config, const char *fname) {
    ...
    if (!config_lookup_int(&cfg, ELAYOUT, &layout)) {
        errno = ENOKEY;
        fatal("can't lookup layout setting.");
        goto out;
    }
    config->layout = (sid_t)layout;

    if (load_volumes_conf(config, &cfg) != 0) {
        fatal("can't load volume config.");
        goto out;
    }

    if (load_exports_conf(config, &cfg) != 0) {
        fatal("can't load exports config.");
        goto out;
    }
