...
*/
dirent_cache_repair_cache_entry_list_t   dirent_cache_repair_cache_entry_list_tab[MDIRENTS_ENTRIES_COUNT+1];
/**
* buffer to rebuild a linked list of hash_entries within a dirent cache entry
*/
dirent_cache_repair_hash_entry_list_t   dirent_cache_repair_hash_entry_list_tab[MDIRENTS_ENTRIES_COUNT+1];
...
{
  int i;
  for (i = 0; i < DIRENT_REPAIR_MAX; i++) dirent_repair_file_stats[i]= 0;
...
void dirent_cache_repair_cache_entry_for_bucket_idx_print(int bucket_idx,char *name)
{
   int i;
   mdirents_hash_entry_t *hash_entry_p;
   if (dirent_repair_print_enable == 0) return; // FDL DEBUG 
   ...
   for (i = 0; i < MDIRENTS_ENTRIES_COUNT; i++)
   {
     ...
     printf(" Local Idx :%3.3d [t:%d idx %3.3d]",dirent_cache_repair_hash_entry_list_tab[i].local_idx,
                    hash_entry_p->next.type,
...
void dirent_cache_repair_printf_cache_entry_for_bucket_idx(mdirents_cache_entry_t *cache_entry_p,int bucket_idx,int coll_idx)
{
   ...
   memset(dirent_cache_repair_hash_entry_list_tab,0,sizeof(dirent_cache_repair_hash_entry_list_t)*MDIRENTS_ENTRIES_COUNT);
   ...
   */
   while(hash_entry_idx < MDIRENTS_ENTRIES_COUNT)
   {
    ...
    dirent_cache_repair_cache_entry_for_bucket_idx_print(bucket_idx,"AFTER REPAIR");
    printf("\n\n");
...
void dirent_file_repair(int dir_fd ,mdirents_cache_entry_t *root_entry_p,int bucket_idx,dirent_file_repair_cause_e cause)
{
    ...
    memset(dirent_cache_repair_cache_entry_list_tab,0,sizeof(dirent_cache_repair_cache_entry_list_t)*MDIRENTS_ENTRIES_COUNT);
    ...
    */     
    dirent_cache_repair_cache_entry_for_bucket_idx(dir_fd,root_entry_p ,root_entry_p, bucket_idx,&hash_entry_p,&first_index);
    /*
...
//        DIRENT_SEVERE("dirent_file_repair no collision file for index %d error at %d\n",coll_idx,__LINE__);
     ...
     */
     dirent_cache_repair_cache_entry_for_bucket_idx(dir_fd,root_entry_p,cache_entry_p , bucket_idx,&hash_entry_p,&first_index);
     /*
