  ...
  */
  {
    ...
    if ( sect0_p == NULL) goto error;
    memcpy(&sect0_p->header,dirent_hdr_p,sizeof(mdirents_header_new_t));
    memset(&sect0_p->coll_bitmap,0xff,sizeof(mdirents_btmap_coll_dirent_t));
  ...
  }
  {
    ...
    coll_bitmap_hash_full_p = DIRENT_MALLOC(sizeof( mdirents_btmap_coll_dirent_t));
    if ( coll_bitmap_hash_full_p == NULL) goto error;  
    memset(coll_bitmap_hash_full_p,0,sizeof(mdirents_btmap_coll_dirent_t));
    /*
    ** store the virtual pointer
    */
  ...
  */
  {
    ...
    if ( name_bitmap_p == NULL) goto error;
...
int dirent_cache_release_entry(mdirents_cache_entry_t *dirent_entry_p)
{
    ...
    */
    while (coll_idx < MDIRENTS_MAX_COLLS_IDX)
    {
      ...
      */
      dirent_coll_entry_p = dirent_cache_get_collision_ptr(dirent_entry_p,(uint32_t )coll_idx);
      ...
      */
      if (dirent_cache_del_collision_ptr(dirent_entry_p, dirent_coll_entry_p) != NULL)
      {
         /*
                                                            ...
                                                            int                   *local_idx_p)
{
    ...
    */
    ...
    coll_bitmap_hash_full_p = DIRENT_VIRT_TO_PHY_OFF(parent,coll_bitmap_hash_full_p);
    ...
    */
    if (parent->hash_entry_full == 0)
    {
      ...
      */
      hash_entry_bit_idx = DIRENT_CACHE_ALLOCATE_HASH_ENTRY_IDX(&sect0_p->hash_bitmap);
...
//        printf("FDL BUG DIRENT_CACHE_ALLOCATE_HASH_ENTRY_ARRAY line %d for idx %d\n",__LINE__,hash_entry_bit_idx);
        hash_entry_p = DIRENT_CACHE_ALLOCATE_HASH_ENTRY_ARRAY(parent,hash_entry_bit_idx);   
        ...
        hash_bucket_p = DIRENT_CACHE_ALLOCATE_BUCKET_ARRAY(parent,bucket_idx);
    ...
    */
    while (coll_idx < MDIRENTS_MAX_COLLS_IDX)
    {
      ...
      if (coll_idx%8 == 0) 
      {     
        next_coll_idx = check_bytes_val(coll_bitmap_hash_full_p,coll_idx,MDIRENTS_MAX_COLLS_IDX,&loop_cnt,1);
        if (next_coll_idx < 0) break;    
        coll_idx = next_coll_idx;
      }
#endif
      ...
      */
      if ((coll_bitmap_p[chunk_u8_idx] & (1<<bit_idx)) != 0)
      {
        ...
        */
        if (free_coll_idx == -1) free_coll_idx = coll_idx;
        /*
      ...
      */
      dirent_entry = dirent_cache_get_collision_ptr(parent,(uint32_t )coll_idx);
...
//      printf("FDL BUG DIRENT_CACHE_ALLOCATE_HASH_ENTRY_ARRAY line %d for idx %d\n",__LINE__,hash_entry_bit_idx);
      ...
      hash_entry_p = DIRENT_CACHE_ALLOCATE_HASH_ENTRY_ARRAY(dirent_entry,hash_entry_bit_idx);   
      if (hash_entry_p == NULL)
      {
        /*
        ** something wrong, we run out of memory
      ...
      hash_bucket_p = DIRENT_CACHE_ALLOCATE_BUCKET_ARRAY(dirent_entry,bucket_idx);
      if (hash_bucket_p == NULL)
      {
        /*
        ** something wrong, we run out of memory
    ...
    */
    {
      ...
      for (i = 0; i < header.level_index; i++)  header.dirent_idx[i] = parent->header.dirent_idx[i];
      ...
      */
      if (dirent_cache_store_collision_ptr(parent,dirent_entry) != NULL)
      {
        ...
        */
        DIRENT_CACHE_RELEASE_COLL_ENTRY_IDX(coll_bitmap_p,coll_idx);
        printf("FDL ->%s:%d ->Put code to release the dirent_entry context \n",__FILE__,__LINE__);
        return NULL;              
      }      
      /*
...
{ 
   ...
   if (free_p == NULL) 
   {
     if (p->header.level_index == 0) 
     {
       ...
       */
       if (p->hash_entry_full != 0) return;
       *free_p = p;
       return;
     }
     if (dirent_test_chunk_bit(p->header.dirent_idx[1],coll_bitmap_hash_full_p)) return;
     *free_p = p;
     return;
   }     
}
                                                                   ...
                                                                   dirent_cache_search_alloc_t *cache_alloc_p)
{
  ...
  coll_bitmap_hash_full_p = DIRENT_VIRT_TO_PHY_OFF(root,coll_bitmap_hash_full_p);
  if (coll_bitmap_hash_full_p == (uint8_t*)NULL)
  {
  ...
  while(1)
  {
    ...
    if (hash_bucket_p->type == MDIRENTS_HASH_PTR_LOCAL) 
    {
      ...
      */
      ...
      hash_entry_cur_p = (mdirents_hash_entry_t*)DIRENT_CACHE_GET_HASH_ENTRY_PTR_WITH_VIRT(cache_entry_cur,cur_hash_entry_idx,&hash_virt_p);
      if (hash_entry_cur_p == NULL)
      {
        /*
        ** something wrong!! (either the index is out of range and the memory array has been released
        */
        printf("dirent_cache_delete_hash_entry error at %d cur_hash_entry_idx %d \n",__LINE__,cur_hash_entry_idx);
        DIRENT_CACHE_LOOKUP_UPDATE_STATS(coll_cnt);
        return NULL;
      }
#if 0 // FDL_DEBUG 
...
//                      name_entry_p->name);
           ...
           */
           if (fid!= NULL) memcpy(fid, name_entry_p->fid,sizeof(fid_t));
           if (mode_p!= NULL) *mode_p = name_entry_p->type;   
           /*
           ** release the bits relative to the neme entry
           */
           dirent_cache_del_entry_name(cache_entry_cur,hash_entry_cur_p->chunk_idx,hash_entry_cur_p->nb_chunk);        
         }
#endif         
         ...
         */
         if (bucket_idx_used)
         {
            ...
            */
            if ((hash_entry_cur_p->next.type == MDIRENTS_HASH_PTR_EOF) || (hash_entry_cur_p->next.type == MDIRENTS_HASH_PTR_COLL))
            {
              /*
              ** indicates that previous cache entry must be also updated
              */
              *cache_entry_prev_ret   = (cache_entry_cur == root)?NULL:cache_entry_prev;
              ...
              */
              if (cache_entry_cur == root) root->hash_entry_full = 0;
              else
              {
                /*
                ...
                */
                DIRENT_CACHE_SET_COLL_ENTRY_NOT_FULL(coll_bitmap_hash_full_p,cache_entry_cur->header.dirent_idx[1]);              
              }
              /*
              ** release the hash entry on bitmap and eventually release the memory if it was the last
              */         
              DIRENT_CACHE_RELEASE_HASH_ENTRY_IDX(hash_bitmap_p,cur_hash_entry_idx);
              DIRENT_CACHE_RELEASE_HASH_ENTRY_ARRAY(cache_entry_cur,hash_bitmap_p,cur_hash_entry_idx);
    ...
    if (hash_bucket_p->type == MDIRENTS_HASH_PTR_LOCAL) 
    {
      ...
      */
      ...
      printf("%3.3d: hash :%8.8d next->%s:%4.4d\n",cur_hash_entry_idx,hash_entry_cur_p->hash, 
          (hash_entry_cur_p->next.type == MDIRENTS_HASH_PTR_LOCAL)?"LOC":"COL" ,
          hash_entry_cur_p->next.idx);
      }
      /*
      ** try the next entry
      */
...
#if 1
    if (DIRENT_PREAD(fd, dirent_file_p, sizeof (mdirents_file_t), offset) != sizeof (mdirents_file_t)) {
...
#else
{
    ...
    */
    {
      ...
      memcpy(sect0_p,&dirent_file_p->sect0,sizeof(mdirent_sector0_not_aligned_t));
      /*
      ** store the virtual pointer
      */
      val = (uint64_t) (uintptr_t) sect0_p;
    ...
    */
    {
      ...
      memcpy(name_bitmap_p,&dirent_file_p->sect1,sizeof(mdirents_btmap_free_chunk_t));
    ...
    */
    {
      ...
      for (i = 0; i < MDIRENTS_HASH_TB_CACHE_MAX_IDX; i++)
      {
        for (k = 0; k < MDIRENTS_HASH_TB_CACHE_MAX_ENTRY; k++)
        {
           ...
           */
           memcpy( elem_p, file_ptr,sizeof(mdirents_hash_ptr_t)*MDIRENTS_HASH_TB_CACHE_MAX_ENTRY); 
           break;          
    ...
    */
    {
       ...
       */
       ...
       {
          if (p64[index]== bitmap_val) continue;
          /*
          ** there at least one entry that is used: need to allocate a free array
          */
          elem_p = DIRENT_MALLOC(sizeof(mdirents_hash_entry_t)*MDIRENTS_HASH_CACHE_MAX_ENTRY);
          if (elem_p == NULL)
          {
            /*
            ** out of memory
          ...
          ** copy the array into the memory
          */
          memcpy( elem_p, &membyte_p[(DIRENT_HASH_ENTRIES_BASE_SECTOR+index)*MDIRENT_SECTOR_SIZE],MDIRENT_SECTOR_SIZE);                
       }
    ...
    if (dirent_p->header.level_index == 0)
    {
       ...
       */
       ...
       while (coll_idx < MDIRENTS_MAX_COLLS_IDX)
       {
         ...
         */
         if (dirent_cache_store_collision_ptr(dirent_p,dirent_coll_entry_p) != NULL)
         {
    ...
    */
#if 1
    {
      ...
      for (first_chunk_of_array = 0; first_chunk_of_array < MDIRENTS_NAME_CHUNK_MAX_CNT;
...
int  write_mdirents_file(int dirfd,mdirents_cache_entry_t *dirent_cache_p)
{
    ...
    */
    {       
      dirent_file_p = writebck_cache_bucket_get_entry(&dirent_cache_p->writeback_ref,dirent_cache_p->header.dirent_idx[0]);
      if (dirent_file_p != NULL)
      {
        writeback_cache = 1;     
      }
...
//    memset(dirent_file_p,0xbb,sizeof(mdirents_file_t));
    ...
    */
    write_len = sizeof(mdirent_sector0_t)+sizeof(mdirent_sector1_t)+sizeof(mdirent_sector2_t); 
    /*
    ** Copy the sectors 3&4 that contain the hash entries
    */
    {
      ...
      */      
      ...
      {
        hash_entry_p = DIRENT_VIRT_TO_PHY_OFF(dirent_cache_p,hash_entry_p[i]);
        if (hash_entry_p != (mdirents_hash_entry_t*)NULL) hash_entry_last_sector_idx++;
      }
...
//      hash_entry_last_sector_idx = MDIRENTS_HASH_CACHE_MAX_IDX;
      for (i = 0; i < hash_entry_last_sector_idx; i++)
      {
        ...
        */
        dirent_cache_p->hash_entry_p[i].s.dirty = 0;     
        file_ptr += MDIRENTS_HASH_CACHE_MAX_ENTRY;
        write_len += sizeof(mdirents_hash_entry_t)*MDIRENTS_HASH_CACHE_MAX_ENTRY;        
      }
    } 
#endif
    /*
    ...
    */
    if (writeback_cache !=0)
    {
      int ret = writebck_cache_bucket_is_write_needed(&dirent_cache_p->writeback_ref,dirent_cache_p->header.dirent_idx[0]);
      if (ret ==1)
      {
   ...
   */
   {
     ...
     */
     ...
     if (DIRENT_PWRITE(fd, mem_p, MDIRENTS_CACHE_CHUNK_ARRAY_SZ, offset) != MDIRENTS_CACHE_CHUNK_ARRAY_SZ) 
     {
         DIRENT_SEVERE("pwrite failed for file %s: %s, at line %d", pathname, strerror(errno),__LINE__);
         goto error;
     }
