...
int export_stat(export_t * e, ep_statfs_t * st) {
    ...
    struct statfs stfs;
    volume_stat_t vstat;
    START_PROFILING_EID(export_stat,export->eid);
    ...
    status = 0;
out:
    STOP_PROFILING_EID(export_stat,export->eid);
    return status;
}
...
int export_setattr(export_t *e, fid_t fid, mattr_t *attrs, int to_set) {
    ...
    if ((to_set & EXPORT_SET_ATTR_SIZE) && S_ISREG(lv2->attributes.mode)) {
        ...
        uint64_t nrb_new = ((attrs->size + ROZOFS_BSIZE_BYTES(e->bsize) - 1) / ROZOFS_BSIZE_BYTES(e->bsize));
        uint64_t nrb_old = ((lv2->attributes.size + ROZOFS_BSIZE_BYTES(e->bsize) - 1) /
                ROZOFS_BSIZE_BYTES(e->bsize));
...
int export_mknod(export_t *e,uint32_t site_number,fid_t pfid, char *name, uint32_t uid,
    ...
    */
    ...
    if (volume_distribute(e->volume,site_number, &attrs->cid, attrs->sids) != 0)
        goto error;
    attrs->mode = mode;
    attrs->uid = uid;
    attrs->gid = gid;
	      ...
	      */
	      geo_rep_insert_fid(e->geo_replication_tb[1],
            ...
            if (export_update_blocks(e,
                    -(((int64_t) lv2->attributes.size + ROZOFS_BSIZE_BYTES(e->bsize) - 1)
...
int64_t export_write_block(export_t *e, fid_t fid, uint64_t bid, uint32_t n,
    ...
    if (off + len > lv2->attributes.size) {
        // Don't skip intermediate computation to keep ceil rounded
        uint64_t nbold = (lv2->attributes.size + ROZOFS_BSIZE_BYTES(e->bsize) - 1) / ROZOFS_BSIZE_BYTES(e->bsize);
...
static inline int get_rozofs_xattr(export_t *e, lv2_entry_t *lv2, char * value, int size) {
  ...
  DISPLAY_ATTR_INT("BSIZE", ROZOFS_BSIZE_BYTES(e->bsize));  
...
static inline int set_rozofs_xattr(export_t *e, lv2_entry_t *lv2, char * value,int length) {
  ...
  */
  ...
  if (idx == 0) {
    for (idx=0; idx < rozofs_safe; idx++) {
      new_sids[idx] = llv2->attributes.s.attrs.sids[idx];
    }
  }
