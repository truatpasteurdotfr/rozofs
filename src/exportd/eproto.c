...
*   exportd write_block: update the size and date of a file
    ...
    @retval: EP_FAILURE :error code associated with the operation (errno)
*/
epgw_io_ret_t * ep_write_block_1_svc(epgw_write_block_arg_t * arg,
        struct svc_req * req) {
    static epgw_io_ret_t ret;
    export_t *exp;
    DEBUG_FUNCTION;
    ...
    if (!(exp = exports_lookup_export(arg->arg_gw.eid)))
        goto error;
    if ((ret.status_gw.ep_io_ret_t_u.length = export_write_block(exp,
            (unsigned char *) arg->arg_gw.fid, arg->arg_gw.bid, arg->arg_gw.nrb, arg->arg_gw.dist,
            arg->arg_gw.offset, arg->arg_gw.length,(mattr_t *) & ret.parent_attr.ep_mattr_ret_t_u.attrs)) < 0)
        goto error;
    ret.hdr.eid = arg->arg_gw.eid ;  
    ret.status_gw.status   = EP_SUCCESS;
    ret.parent_attr.status = EP_SUCCESS;
