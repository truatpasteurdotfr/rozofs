...
* Buffers information
*/
int geo_replicat_buf_count= 0;   /**< number of buffer allocated for read/write on north interface */
int geo_replicat_buf_sz= 0;      /**<read:write buffer size on north interface */

void *geo_replicat_receive_buffer_pool_p = NULL;  /**< reference of the read/write buffer pool */
void *geo_replicat_xmit_buffer_pool_p = NULL;  /**< reference of the read/write buffer pool */
 

//extern void geo_replica_req_rcv_cbk(void *userRef,uint32_t  socket_ctx_idx, void *recv_buf);
...
*/
void * geo_replicat_north_RcvAllocBufCallBack(void *userRef,uint32_t socket_context_ref,uint32_t len)
{
  /*
  ** We need at least a response buffer
  */
  uint32_t free_count = ruc_buf_getFreeBufferCount(geo_replicat_xmit_buffer_pool_p);  
  if (free_count < 1)
  {
    return NULL;
  }
   ...
   */
   if (len >  geo_replicat_buf_sz)
   {   
     return NULL;   
   }
   return ruc_buf_getBuffer(geo_replicat_receive_buffer_pool_p);      
}
...
*/
uint32_t geo_replicat_north_userRcvReadyCallBack(void * socket_ctx_p,int socketId)
{
...
* test function that is called upon a failure on sending
 ...
 @retval none
*/
void  geo_replicat_north_userDiscCallBack(void *userRef,uint32_t socket_context_ref,void *bufRef,int err_no)
{
 ...
 */
 af_unix_socket_conf_t  georep_af_inet_rozofs_north_conf =
{
  ...
  geo_replicat_north_RcvAllocBufCallBack,  //    userRcvAllocBufCallBack; /* user callback for buffer allocation */
  geo_replica_req_rcv_cbk,           //    userRcvCallBack;   /* callback provided by the connection owner block */
  geo_replicat_north_userDiscCallBack,   //    userDiscCallBack; /* callBack for TCP disconnection detection         */
  ...
  geo_replicat_north_userRcvReadyCallBack,  //    userRcvReadyCallBack; /* NULL for default callback                     */
...
int geo_replicat_north_interface_buffer_init(int read_write_buf_count,int read_write_buf_sz)
{
    ...
    geo_replicat_buf_count  = read_write_buf_count;
    geo_replicat_buf_sz     = read_write_buf_sz    ;
    
    georep_af_inet_rozofs_north_conf.rpc_recv_max_sz = geo_replicat_buf_sz;
    
    /*
    ** create the pool for receiving requests from rozofsmount
    */
    geo_replicat_receive_buffer_pool_p = ruc_buf_poolCreate(geo_replicat_buf_count, geo_replicat_buf_sz);
    if (geo_replicat_receive_buffer_pool_p == NULL)
    {
    ...
    */
    geo_replicat_xmit_buffer_pool_p = ruc_buf_poolCreate(geo_replicat_buf_count, geo_replicat_buf_sz);
    if (geo_replicat_xmit_buffer_pool_p == NULL)
    {
       severe( "ruc_buf_poolCreate(%d,%d)", geo_replicat_buf_count, geo_replicat_buf_sz ); 
       return -1;
    }
...
int geo_replicat_north_interface_init(char *host,uint16_t port) {
  ...
  */ 
  ret = af_inet_sock_listening_create("GEOREP",
                                      ipaddr,
				      port,
				      &georep_af_inet_rozofs_north_conf);
  if (ret < 0) {
    uint32_t ip = ipaddr;
