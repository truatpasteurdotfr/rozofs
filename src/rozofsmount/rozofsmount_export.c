...
#define SHOW_PROFILER_PROBE(probe) pChar += sprintf(pChar," %12s | %15lu | %9lu | %15lu\n",\
					#probe,\
...
void show_profiler(char * argv[], uint32_t tcpRef, void *bufRef) {
    ...
    SHOW_PROFILER_PROBE(read);
    SHOW_PROFILER_PROBE(write);
...
static struct fuse_lowlevel_ops rozofs_ll_operations = {
    ...
    .listxattr = rozofs_ll_listxattr,    /** !!! blocking !!! */
...
//    uint16_t debug_port = 60000;
    rozofs_fuse_conf.debug_port = 60000;
...
#endif
#if 0 // remove because of non blocking FDL
