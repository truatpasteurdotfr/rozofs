...
typedef struct _rozofs_shared_pool_t
{
   ...
   uint64_t read_stats; /**< read statistics: incremented each time it is allocated     */
...
*  Allocate a packet buffer from a shared memory for a given storcli
   ...
   @param storcli_id : index of the storcli 
   
   @retval <>NULL reference of the buffer
   @retval NULL : no buffer
*/
static inline void  *rozofs_alloc_shared_storcli_buf(int storcli_idx)
{
   if (storcli_idx >= STORCLI_PER_FSMOUNT)
   {
      severe("rozofs_alloc_shared_storcli_buf: storcli %d out of range",storcli_idx);
      return NULL;
   }
   if (rozofs_storcli_shared_mem[storcli_idx].key == 0)
   {
      /*
   ...
   */  
   rozofs_storcli_shared_mem[storcli_idx].read_stats++; 
   return (ruc_buf_getBuffer(rozofs_storcli_shared_mem[storcli_idx].pool_p));
}
...
*/
static inline int rozofs_get_shared_storcli_payload_idx(void *ruc_buf,int storcli_idx,uint32_t *length_p)
{
   int buffer_idx;
   ...
   */   
   if (storcli_idx >= STORCLI_PER_FSMOUNT)
   {
      severe("rozofs_get_shared_storcli_payload_idx: storcli %d out of range",storcli_idx);
      return -1;
   }
   if (rozofs_storcli_shared_mem[storcli_idx].key == 0)
   {
   ...
   char *data_p = rozofs_storcli_shared_mem[storcli_idx].data_p;
   buffer_idx = ( payload - data_p)/rozofs_storcli_shared_mem[storcli_idx].buf_sz;
   if (buffer_idx > rozofs_storcli_shared_mem[storcli_idx].buf_count)
   {
      severe("rozofs_get_shared_storcli_payload_idx: bad pointers");
      return -1;   
   }
   *length_p = rozofs_storcli_shared_mem[storcli_idx].buf_sz;
   return buffer_idx;
}
